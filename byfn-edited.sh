...
function networkUp () {
  ...
  docker exec cli scripts/edited-script.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT
  if [ $? -ne 0 ]; then
...
function upgradeNetwork () {
  ...
  for PEER in peer0.org1.example.com peer1.org1.example.com peer0.org2.example.com peer1.org2.example.com peer0.org3.example.com peer1.org3.example.com peer0.org4.example.com peer1.org4.example.com peer0.org5.example.com peer1.org5.example.com; do
...
function networkDown () {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts/*.block channel-artifacts/*.tx crypto-config ./org3-artifacts/crypto-config/ channel-artifacts/org3.json ./org4-artifacts/crypto-config/ channel-artifacts/org4.json ./org5-artifacts/crypto-config/ channel-artifacts/org5.json ./org6-artifacts/crypto-config/ channel-artifacts/org6.json
    # remove the docker-compose yaml file that was customized to the example
    rm -f docker-compose-e2e.yaml
  fi
}

#Kill 6 peers and their cc containers
function simulateDisaster() {
  #Get all peer containers
