...
export class HealthRecordInvokes extends FabricConnection {
  ...
  async createRecord(record: MedicalRecord) {
    ...
    try {
      ...
      this.tx_id = this.fabricClient.newTransactionID();
      ...
      var request = {
        ...
        args: [record._id, record.name, record.lastname, record.gender, record.address, record.contactPerson],
        chainId: config.channel.NAME,
        txId: this.tx_id
      };
      ...
      if (isProposalGood) {
        ...
        let transaction_id_string = this.tx_id.getTransactionID(); //Get the transaction ID string to be used by the event processing
        let promises = [];
...
//     chaincodeId: 'fabcar',
//     fcn: 'createCar',
//     args: [record._id, record.make, record.model, record.colour, record.owner],
//     chainId: 'mychannel',
//     txId: tx_id
//   };
